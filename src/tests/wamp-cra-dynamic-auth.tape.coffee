...
# ------------------------------------------------------------------------------
test('Dynamic WAMP-CRA : establish a new session', (assert)->
    ...
    ROUTER_CONFIG.auth.wampcra.authenticator = authenticator
...
test('Static WAMP-CRA : fail to establish a new session - invalid authid', (assert)->
    ...
    authenticator = (realm, authid, details)->
        if authid==VALID_AUTHID
            { secret: VALID_SECRET, role: ROLE }
        else
            null
...
# ------------------------------------------------------------------------------
# Fail to establish a new session - onChallenge incorrectly signs
# ------------------------------------------------------------------------------
test('Static WAMP-CRA : fail to establish a new session - onChallenge incorrectly signs', (assert)->
    ...
    onChallenge = (session, method, extra)->
        ...
        # respond to the challenge
        #
        autobahn.auth_cra.sign(VALID_SECRET, {a: 'this is not right'})
