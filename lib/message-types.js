(function() {
  ...
  Message = (function() {
    ...
    Message.prototype.set = function(path) {
      ...
      if (path.match(/kwargs/) && (value == null)) {
        value = {};
      } else if (path.match(/args/) && (value == null)) {
        value = [];
      }
      if (value != null) {
  ...
  TYPES = {
    ...
    36: {
      ...
      decode: function(data) {
        return new Message(data).set('subscribed.subscription.id').set('published.publication.id').set('details').set('publish.args').set('publish.kwargs').get();
      }
    },
    48: {
      type: 'CALL',
