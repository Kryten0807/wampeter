(function() {
  ...
  Authenticator = (function() {
    ...
    function Authenticator(session1, config) {
      ...
      this._wampcra_compute_challenge = bind(this._wampcra_compute_challenge, this);
      ...
      try {
        ...
        logger.debug('built list of users', this.users);
    ...
    Authenticator.prototype._compute_wcs = function(key, challenge) {
      var deferred, hmac;
      deferred = q.defer();
      hmac = crypto.createHmac('sha256', key);
      hmac.setEncoding('binary');
      hmac.end(challenge, 'utf8', function() {
        var s;
        s = (new Buffer(hmac.read())).toString('base64');
        logger.debug('computed hash', s);
    ...
    Authenticator.prototype._wampcra_compute_challenge = function(user) {
      var challenge, sessionDetails;
      sessionDetails = {};
      ...
      logger.debug('about to issue challenge', challenge);
      return this._compute_wcs(user.secret, challenge).then(function(signature) {
        var extra, ref, ref1;
        extra = {
          challenge: challenge
        };
        if (user.salt != null) {
          ...
          extra.iterations = (ref = user.iterations) != null ? ref : 1000;
          extra.keylen = (ref1 = user.keylen) != null ? ref1 : 32;
        }
        return {
          extra: extra,
          signature: signature
